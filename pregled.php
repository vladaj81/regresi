...
<head>
	...
	<script type="text/javascript">
		...
		function prikazi_podatke_osiguravajuceg_drustva(id) {
			...
			//DODAO VLADA/PROJEKAT REGRESI - ZA RESET LISTE SA MESTIMA I OSTALIH INPUT POLJA NA PROMENU OSIGURANJA
			$('#mesto_reg').html('');
			$('#jmbg_pib').val('');
			$('#adresa_reg').val('');
			$('#telefon_reg').val('');
			...
			//DODAO VLADA USLOV/PROJEKAT REGRESI
			if (id != 26 && id != -1) {
			...
			$.ajax({
				...
				success: function(ret) {
					...
					//DOHVATANJE SELECT LISTE SA DRZAVAMA PO ID-JU - DODAO VLADA/PROJEKAT REGRESI
			...
			$.widget("ui.combobox", {
				_create: function() {
					var input, self = this,
						...
						//SETOVANJE VISINE TEXTAREA SA RAZLOGOM SMANJENJA STETE, U ZAVISNOSI DA LI IMA VREDNOSTI - DODAO VLADA/PROJEKAT OSNOVAN/DELIMICNO/ODBIJEN
						var visina = (value == '') ? 'height:30px;' : 'height:60px;';

					//DODAO VLADA/PROJEKAT OSNOVAN/DELIMICNO/ODBIJEN
					input = $("<textarea id='dodatno' style='margin-left:0px;width:420px;color:black;font-size:12px; " + visina + " vertical-align: middle;'></textarea>").appendTo(wrapper).val(value).autocomplete({
						delay: 1000,
						minLength: 3,
						...
						select: function(event, ui) {
							...
							//DODAO VLADA/PROJEKAT OSNOVAN/DELIMICNO/ODBIJEN -ZBOG KOREKCIJE VISINE POLJA SA RAZLOGOM SMANJENJA STETE
							$('#dodatno').css('height' , '60px');
						},
					...
					//DODAO VLADA/PROJEKAT OSNOVAN/DELIMICNO/ODBIJEN - ZA RESETOVANJE TEXTAREA SA RAZLOGOM SMANJENJA STETE
					if ($('#osnovan_ceo').is(':checked')) {

						$('#dodatno').val('');
					}

					$("#dodatno")._renderItem = function(ul, item) {
						return $("<li></li>").data("item.autocomplete", item).append("<a>" + item.label + "</a>").appendTo(ul);
					};

					$("#dodatno")._renderMenu = function(ul, items) {
						var self = this,
							currentCategory = "";
		...
		function toggle_dugme_resenje_odbijen(id) {
			...
			//DODAO VLADA/PROJEKAT OSNOVAN/DELIMICNO/ODBIJEN - DEO KODA ZA ODCEKIRANJE CHECKBOXA ODUSTAO I SAKRIVANJE DUGMETA RESENJE ODUSTAO
			$('#odustao_pravni_osnov').prop('checked', false);
			$('#resenje_odustao').hide();

			//DODAO VLADA/PROJEKAT OSNOVAN/DELIMICNO/ODBIJEN - ZBOG PROMENE VISINE TEXTAREA RAZLOG SMANJENJA STETE
			if (id != 'osnovan_delimicno') {

				$('#dodatno').css('height', '30px');
			}
			...
			if (id == "osnovan_delimicno" && id != "" && id != "odbijen") {
				...
				//DODAO VLADA/PROJEKAT OSNOVAN/DELIMICNO/ODBIJEN - ZBOG RESETOVANJA POLJA KAD NIJE ODABRANO OSNOVAN DELIMICNO
		...
		function JesiSiguran() {
			if (jeStorno == 1) {
				...
				if (siguranSam == true) {
					...
					//DODAO VLADA/PROJEKAT OSNOVAN/DELIMICNO/ODBIJEN - POCETAK
					...
					//DODAO VLADA/PROJEKAT OSNOVAN/DELIMICNO/ODBIJEN - KRAJ
				} else {
		...
		//FUNKCIJA NA PROMENU VREDNOSTI SELECTA SA DRZAVAMA - DODAO VLADA/PROJEKAT REGRESI
		...
		//FUNKCIJA ZA PRIKAZ I SAKRIVANJE POLJA U ZAVISNOSTI OD TIPA SELEKTOVANOG REGRESNOG DUZNIKA - DODAO VLADA/PROJEKAT REGRESI
		function promena_input_polja(regres_od) {
			...
			if(regres_od == 'Osiguravajuæe dru¹tvo') {
				...
				//AKO JE PRAVNO LICE,PROMENI MAXLENGTH
				if(tip_lica == 'pravno') {
...
*/
		...
		//DODAO VLADA/PROJEKAT REGRESI - UZIMANJE VREDNOSTI IZ BAZE ZA UPIS U POLJA
		...
		//DODAO VLADA/PROJEKAT REGRESI - KRAJ
		...
		//DODAO VLADA/PROJEKAT REGRESI - ZBOG PROVERE DA LI TREBA VRSITI VALIDACIJU U SEKCIJI PRAVNI OSNOV
		echo "<input type='hidden' name='osnov_za_regres_baza' value= '$potvrdjen_osnov_za_regres' />";
...
*/
		...
		//IZMENIO VLADA/PROJEKAT REGRESI
		...
		//DODAO VLADA/PROJEKAT REGRESI - ZA RAZLICITE SIRINE INPUT I SELECT POLJA
		$sirina_polja = ($vrstaSt == 'AK') ? '180px' : '200px';


		//PREMESTIO VLADA/PROJEKAT REGRESI
		...
		//VLADA IZMENIO COLSPAN/PROJEKAT REGRESI
		if ($vrstaSt == 'AK') {
		...
		//DODAO VLADA KLASU PADDING/PROJEKAT REGRESI
		...
		//PREMESTIO VLADA/PROJEKAT REGRESI - KRAJ
		...
		//IZMENIO VLADA - DODAO VLADA ONKEYPRESS FUNKCIJU SAMO BROJEVI /PROJEKAT REGRESI
		...
		//UPIT ZA DOBIJANJE DRZAVA IZ SIFARNIKA - DODAO VLADA /PROJEKAT REGRESI
		...
		//IZMENIO VLADA I PREMESTIO LABELU /PROJEKAT REGRESI
		echo "<td class=\"dodaj_padding\" bgcolor=\"#2BAEFF\">\n";
		...
		//PROLAZAK KROZ NIZ SA DRZAVAMA - DODAO VLADA /PROJEKAT REGRESI
		...
		//DOBIJANJE OPSTINA IZ SIFARNIKA - DODAO VLADA /PROJEKAT REGRESI
		$niz_opstine = pg_fetch_all($rezultatOpstine);

		//PROLAZAK KROZ NIZ SA OPSTINAMA - DODAO VLADA /PROJEKAT REGRESI
		...
		if($opstina_reg_id != '' && $mesto_reg_id != '') {
			...
			//DOBIJANJE MESTA IZ SIFARNIKA PO ID-JU OPSTINE - DODAO VLADA /PROJEKAT REGRESI
			$rezultatMesta = $sifarnici_class->vratiMesta($opstina_reg_id );
			$niz_mesta = pg_fetch_all($rezultatMesta);
		
			//PROLAZAK KROZ NIZ SA MESTIMA - DODAO VLADA /PROJEKAT REGRESI
		...
		echo "</tr>\n";
		//IZMENA VLADA - KRAJ /PROJEKAT REGRESI
...
value='Re¹enje odustao' $hidden_pravni>";
		...
		// obracun ao lica
		$sql_obracun_ao_lica = "SELECT * FROM obracun_visine_stete_ao_lica WHERE predmet_odstetnog_zahteva_id = $idstete ORDER BY id DESC LIMIT 1";
		...
		$html_sporazum_ao 	  = $podaci_obracun_ao_lica["html_sporazum_ao"];
		...
		// sporazum ao
		if($html_sporazum_ao != null)
		{
			$za_stampu_sporazum_ao = '<td><input type="checkbox" id="modal_sporazum_ao_lica" name="modal_sporazum_ao_lica" value="11" checked /></td>'; 
		}
		else 
		{
			$za_stampu_sporazum_ao = '<td><input type="checkbox" id="modal_sporazum_ao_lica" name="modal_sporazum_ao_lica" value=11 disabled /></td>';
		}

		// obracun 0205 dpz
		$sql_obracun_0205_dpz = "SELECT * FROM obracun_visine_stete_0205_dpz WHERE idstete = $idstete ORDER BY id DESC LIMIT 1";
		...
		// obracun n dpz
		$sql_obracun_n_dpz = "SELECT * FROM obracun_visine_stete_n_dpz WHERE predmet_odstetnog_zahteva_id = $idstete ORDER BY id DESC LIMIT 1";
...
// 2021-05-20 tipSt Mare
		...
		else if($tipSt == '1001011' || $tipSt == '1001021' || $tipSt == '1001031' || $tipSt == '1001041' || $tipSt == '1001051' || $tipSt == '1001061' || $tipSt == '1001071' || $tipSt == '1001081' || $tipSt == '1001091' || $tipSt == '1001101' || $tipSt == '1001111' || $tipSt == '1001121' || $tipSt == '1001131' || $tipSt == '1001141')
		{
			echo "<td>$za_stampu_resenja</td><td>Re¹enja</td>";
			echo "<td>$za_stampu_instrukcije</td><td>Instrukcije</td>";
			echo "<td>$za_stampu_obracun_ao_lica</td><td>Obraèun visine ¹tete ao lica </td>";
			echo "<td>$za_stampu_sporazum_ao</td><td>Sporazum </td>";
		}
		else if($tipSt == '0103' || $tipSt == '0105' || $tipSt == '010101' || $tipSt == '010201' || $tipSt == '010103' || $tipSt == '010203' || $tipSt == '020201' || $tipSt == '020202')
		{
			echo "<td>$za_stampu_resenja</td><td>Re¹enja</td>";
			echo "<td>$za_stampu_instrukcije</td><td>Instrukcije</td>";
		...
		// Mare 2021-06-22
		echo "<input type='button' id='provera_dokumentacije' name='provera_dokumentacije' onclick='provera_dokumentacije_likvidacija()' value='Pregled dokumenata' />";
		// echo "<a href=pregled_dokumentacije_likvidacija.php?idstete=$idstete target='_blank' >Pregled dokumenata </a>";
		echo "&nbsp&nbsp&nbsp";
...
*/
    ...
    VALUES ( $idstete, ";
			...
			/*DODATO VLADA ZBOG TESTIRANJA - MOZE DA SE SKLONI
			echo "<script language=\"javascript\">\n";
			echo "alert('$osnov_za_regres_baza');";
			echo "</script>\n";
			*/
			...
			//AKO NIJE POTVRDJEN OSNOV ZA REGRES U BAZI,POKRENI VALIDACIJU
			if($osnov_za_regres_baza != 't') {

				//AKO JE KLIKNUTO NA RADIO BUTTON POTVRDJEN OSNOV ZA REGRES - DA
				...
				if ($nastavi === true) {
					...
					if ($datumPravniOsnov || $osnovan || $delimicnoProc || $vraceno || $vrstaRegPotr || $osiguranjeRegPotr || $drzavaRegPotr || $regPotr || $pravnaPredato || $pravniOsnovDao != '0' || $pravniOsnovDao_1 != '0' || $pravniOsnovDao_2 != '0'  ||$pravniOsnovNapomena || $pravniOsnovObradjivac != '0' || $datumPrijemaPredmetaPravnaSluzba || $pravniOsnovDatumKompletiranjaDokumentacije || $razlog_umanjenja_stete_id || $lista_za_pravni_osnov || $alkotest_osteceni || $alkotest_krivac || $regres_od || $osiguravajuce_drustvo_id || $potvrdjen_osnov_za_regres || $radnik_evidentirao_potvrdu_za_regres || $datum_evidentiranja_potvrde_za_regres || $razlog_regresa_id || $regresno_potrazivanje_napomena || $odustao_pravni_osnov) {
						...
						*/	
						...
						//DODAO VLADA PROSIRENJE UPITA ZA NOVE KOLONE U TABELI PRAVNI - POCETAK /PROJEKAT REGRESI
						if($tip_lica) {
						...
						//DODAO VLADA - KRAJ /PROJEKAT REGRESI
  ...
  <input type='radio' name='razbijanje' value='razbijanje_isplata' id='razbijanje_isplata' onclick='prikazi_div(this.value)'>ISPLATA";
	...
	//DODAO VLADA - SACEKAJ DA SE UCITA STRANICA /PROJEKAT REGRESI
	$(document).ready(function() {
		/*
		...
		*/
		...
		//AKO JE TIP LICA FIZICKO,PROMENI MAXLENGTH ATRIBUT
		if (vrsta_lica == 'fizicko') {  

			$("#jmbg_pib").attr('maxlength', 13);
		}

		//AKO JE TIP LICA PRAVNI,PROMENI MAXLENGTH ATRIBUT
		...
		//FUNKCIJA NA PROMENU RADIO BUTTONA - DODAO VLADA /PROJEKAT REGRESI
	...
	//FUNKCIJA ZA PROVERU DA LI JE DATUM U ISPRAVNOM FORMATU - DODAO VLADA /PROJEKAT REGRESI
	...
	//FUNKCIJA ZA PROVERU DA LI JE JMBG VALIDAN - DODAO VLADA /PROJEKAT REGRESI
	...
	//FUNKCIJA ZA PROVERU DA LI JE PIB U DOBROM FORMATU - DODAO VLADA /PROJEKAT REGRESI
	function proveri_pib(b) {
		try {
			var a = b.value;
		}
	...
	function stampaj_generisana_dok()
	{
		...
		var uslov_sporazum_ao;
		if ($('#modal_sporazum_ao_lica').is(':checked')) 
		uslov_sporazum_ao = "DA";
		else uslov_sporazum_ao = "NE";
		...
		$.ajax({
			...
			data: { poz_id: poz_id,
					...
					uslov_sporazum_ao: uslov_sporazum_ao
				},
			datatype: 'json',
					success: function(ret)
					{
	...
	// Mare 2021-06-22 provera da li postoje dokumenti likvidacije za kreiranje horizontalnih slika i prikaz slika ako postoje
	function provera_dokumentacije_likvidacija()
	{
		var poz_id = document.pregled.idstete.value;
		$.ajax({
			type:'POST',
			url: "generisana_dokumentacija_funkcije.php?funkcija=provera_dokumentacije_likvidacija",
			data: { poz_id: poz_id },
			datatype: 'json',
					success: function(ret)
					{
						...
						{
							window.open('pregled_dokumentacije_likvidacija.php?idstete='+poz_id, '_blank');
						}
					}	
	});
	}
